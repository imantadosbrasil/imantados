(() => {
  ...
  function validateCardForm() {
    ...
    const email = (cardEmailEl?.value || '').trim();
    ...
    clearInvalid(cardEmailEl, errEmailEl);
    ...
    if (!email || !email.includes('@')) { setInvalid(cardEmailEl, errEmailEl); ok = false; }
  ...
  cardEmailEl?.addEventListener('input', function(){ clearInvalid(cardEmailEl, errEmailEl); });
  ...
  (function prefillIfExists(){
    ...
    let order = null; try { order = JSON.parse(localStorage.getItem('lastOrder') || 'null'); } catch {}
    ...
    // Preenche email do order se existir
    if (order && order.customer && order.customer.email && cardEmailEl) {
      cardEmailEl.value = order.customer.email;
    }
  })();
  ...
  btnCardContinue?.addEventListener('click', function(){
    ...
    // Atualizar order com email e CPF informados
    try {
      let order = JSON.parse(localStorage.getItem('lastOrder') || 'null');
      if (order && order.customer) {
        order.customer.email = (cardEmailEl?.value || '').trim();
        order.customer.cpf = digitsOnly(docNumberEl?.value || '');
